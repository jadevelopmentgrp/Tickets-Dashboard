<script>
	...
	async function dispatchConfirm() {
		...
		if (transcriptFileInput.files.length > 0) {
			const presignTranscriptRes = await axios.get(
				`${API_URL}/api/${guildId}/import/presign?file_size=${transcriptFileInput.files[0].size}&file_type=transcripts&file_content_type=${transcriptFileInput.files[0].type}`
			);
			...
			await fetch(presignTranscriptRes.data.url, {
				...
				.then((res) => {
					...
					dataReturned = true;
					notifySuccess(
						'Transcripts uploaded successfully - They have now been placed in a queue and will be processed over the next few days.'
					);
				})
				.catch((e) => {
		...
		if (dataFileInput.files.length > 0) {
			const presignDataRes = await axios.get(
				`${API_URL}/api/${guildId}/import/presign?file_size=${dataFileInput.files[0].size}&file_type=data&file_content_type=${dataFileInput.files[0].type}`
			);
			...
			await fetch(presignDataRes.data.url, {
				...
				.then((res) => {
					...
					dataReturned = true;
					notifySuccess(
						'Data uploaded successfully - It has now been placed in a queue and will be processed over the next few days.'
					);
				})
				.catch((e) => {
		...
		<div slot="body" class="body-wrapper">
						...
						<div class="col-6">
							<Button
								...
								icon={queryLoading
									...
									: ''}
								disabled={queryLoading}>Confirm</Button
							>
						</div>
					</div>
					<div class="row">
						{#if queryLoading}
							<div>
								...
								<p style="text-align: center;">
									...
									></i> We are currently loading your data in, please
			...
			{#if runs.length > 0}
				<div class="section">
					<h2 class="section-title">
						Runs <span style="font-size: 12px; font-style: italic;"
							>(Refreshes every 30 seconds)</span
						>
					</h2>
					{#each ['DATA', 'TRANSCRIPT'] as runType}
						{#if runs.filter((run) => run.run_type == runType).length > 0}
							<h3>
								{runType
									.toLowerCase()
									.replace(/\b\w/g, (s) => s.toUpperCase())} Logs
							</h3>
							{#each runs.filter((run) => run.run_type == runType) as run}
								<Collapsible tooltip="View your logs for this run">
									<span slot="header" class="header"
										>{run.run_type} Run #{run.run_id} - {new Date(
											run.date
			...
			{#if dataReturned}
				<div class="section">
					...
					<div class="row">
						<p style="text-align: center;">
							Your Data & Transcripts have been placed in a queue and may take

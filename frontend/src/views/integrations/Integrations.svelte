<script>
	...
	withLoadingScreen(async () => {
		...
		await Promise.all([loadOwnedIntegrations(), loadAvailableIntegrations()]);
...
<div class="content">
	<div class="container">
		<div class="spread">
			...
			<Button
				icon="fas fa-server"
				on:click={() => navigateTo(`/manage/${guildId}/integrations/create`)}
				>Create Integration
			</Button>
		</div>
		...
		<div class="integrations">
			...
			{#if page === 1}
				<div class="integration">
					...
					>
						<span slot="description">
							Our Bloxlink integration inserts the Roblox usernames, profile
							URLs and more of your users into ticket welcome messages
							automatically! This integration is automatically enabled in all
							servers, press the View button below to check out the full list
							of placeholders you can use!
						</span>
					</Integration>
				</div>
			{/if}
			...
			{#each availableIntegrations as integration}
				<div class="integration">
					<Integration
						...
						on:remove={() => removeIntegration(integration.id)}
					>
						<span slot="description">
							{integration.description}
						</span>
